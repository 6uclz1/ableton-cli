...
def _write_quality_harness_report(path: Path, *, status: str, warnings: int, failures: int) -> None:
    path.write_text(
        json.dumps(
            {
                "summary": {
...
def test_main_lists_failed_tests_with_file_test_name_and_message(tmp_path: Path) -> None:
    ...
    junit_report = tmp_path / "pytest-report.xml"
    summary_path = tmp_path / "summary.md"
    _write_quality_harness_report(quality_report, status="fail", warnings=1, failures=2)
